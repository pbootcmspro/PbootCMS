...
function login_black_proc_ok($proc)
{
...
function login_black_command(string $ip, string $runPath, int $lockTime, int $lockCount, int $lockMax, string $mode): string
{
    ...
    // Windows PHP 7.x：勿对 PHP 可执行文件 escapeshellarg，否则 proc_open 报路径语法错误
    $args = array($script, $ip, $runPath, (string) $lockTime, (string) $lockCount, (string) $lockMax, $mode);
...
function login_black_decode_stdout(string $stdout, string $label)
{
...
return TestAssert::runSuite(function () {
    ...
    $checkRun = login_black_run('9.9.9.9', $runPath, 900, 5, 1000, 'check');
    TestAssert::same(0, $checkRun['exit_code'], 'corrupted-file probe exited 0');
    TestAssert::true($checkRun['stdout'] !== '', 'corrupted-file probe produced JSON output (no silent crash)');
    TestAssert::true(strpos($checkRun['stderr'], 'Fatal error') === false, 'corrupted-file probe: no fatal error');
    $decoded = login_black_decode_stdout($checkRun['stdout'], 'corrupted-file probe: valid JSON output despite corrupt data file');
    if (is_array($decoded)) {
    ...
    foreach (array_keys($malformed) as $ip) {
        ...
        $checkResult = login_black_decode_stdout($checkRun['stdout'], 'malformed entry check returned valid JSON: ' . $ip);
    ...
    if (isset($rebuilt['9.9.9.9']) && is_array($rebuilt['9.9.9.9'])) {
    ...
    $evictionResult = login_black_decode_stdout($evictionRun['stdout'], 'eviction probe returned valid JSON');
    if (is_array($evictionResult)) {
